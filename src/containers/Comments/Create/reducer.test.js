...
describe('Comment Reducers', () => {
  it('should set isFetching true when COMMENTS_POST_REQUEST is dispatched', () => {
    const action = actions.createComment();

    const newState = createCommentReducer(initialState, action);

    expect(Object.keys(newState).length).toEqual(6);
    expect(newState.isFetching === true);
  });

  it('should set failure True when COMMENTS_POST_FAILURE', () => {
    const errors = {
      errors: {
    ...
    expect(Object.keys(newState).length).toEqual(7);
    expect(newState.failure).toBe(true);
  });

  it('should set success True when COMMENTS_POST_SUCCESS', () => {
    const payload = {
      comment: {
        count: 1,
        next: null,
    ...
    const action = actions.commentSuccess(payload);
