...
describe('<SignUpForm/>', () => {
  ...
  it('should return validation message if username is invalid', () => {
    ...
    wrapper.find('#username').simulate('change', event);
    expect(wrapper.state('validation').username).toEqual('Username should have at least 3 letters');
  });
  ...
  it('should return validation message if email is invalid', () => {
    ...
    wrapper.find('#email').simulate('change', event);
    expect(wrapper.state('validation').email).toEqual('Email should have the format user@mail.com');
  });
  ...
  it('should run handleClick method', () => {
    const wrapper = shallow(<SignUpForm register={{ failure: false }} onClick={() => {}} />);
